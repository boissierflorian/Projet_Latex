%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Boissier Florian
% Fichier personel principal: boissier/rapport.tex
% L3 Info
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Quaternions et rotation dans l'espace}
\input{boissier/subsections/introduction.tex}

\section{Opérations de rotation à l'aide de quaternions}
	\input{boissier/subsections/intro_operations_rotations.tex}
	\subsection{L'hypersphère des rotations}
		\subsubsection{Se faire une idée de l'espace des rotations}
			\input{boissier/subsections/idee_espace_rotations.tex}
		\subsubsection{Paramétrer l'espace des rotations}
			\input{boissier/subsections/parametrer_espace_rotations.tex}
	\subsection{Des rotations aux quaternions}
		\subsubsection{Les quaternions en bref}
			\input{boissier/subsections/quaternions_bref.tex}
		\subsubsection[Rel. entre rotations et quaternions]{Relation entre les rotations et les quaternions unitaires}
			\input{boissier/subsections/relation_rotations_quaternions.tex}
		\subsubsection[Dem. equiv. quaternions et rotation de l'espace]{Démonstration de l'équivalence entre conjugaison de quaternions et rotation de l'espace}
			\input{boissier/subsections/dem_equivalence.tex}
	\subsection{Exemple}
		\input{boissier/subsections/exemple.tex}
	
\section{Expliquer les propriétés des quaternions à l'aide des rotations}
	\subsection{Non-commutativité}
		\input{boissier/subsections/non_commutativite.tex}
	\subsection{Les quaternions sont-ils orientés ?}
		\input{boissier/subsections/quaternions_orientes.tex}
	
\section{Les quaternions et les autres représentations des rotations}
	\subsection{Description qualitative des avantages des quaternions}
		\input{boissier/subsections/description_qualitative.tex}
	\subsection{Conversion vers et depuis la représentation sous forme de matrice}
		\subsubsection{D'un quaternion en matrice orthogonale}
			\input{boissier/subsections/quaternion_matrice.tex}
		\subsubsection{D'une matrice orthogonale en quaternion}
			\input{boissier/subsections/matrice_quaternion.tex}
		\subsubsection{Quaternions optimaux}
			\input{boissier/subsections/quaternions_optimaux.tex}
	\subsection[Performances autres rotations]{Comparaisons de performances avec d'autres méthodes de rotation}
		\input{boissier/subsections/comparaisons_performances.tex}
		\input{boissier/subsections/resultats.tex}
		\subsubsection{Méthodes utilisées}
			\input{boissier/subsections/methodes_utilisees.tex}
			\paragraph{Pseudo-code pour la rotation à l'aide de quaternions}
				\input{boissier/subsections/pseudoc_rot_quaternions.tex}
			\paragraph{Pseudo-code pour créer une matrice angle et axe}
				\input{boissier/subsections/pseudoc_mat.tex}
				
\section[Les paires de quaternions unitaires]{Les paires de quaternions unitaires comme rotations dans l'espace à 4 dimensions}
	\input{boissier/subsections/paires_quaternions.tex}